...
import { Pressable, StyleSheet, Text, View, ViewStyle, TouchableOpacity, StyleProp } from 'react-native';
...
const badgeColors: Record<string, string> = {
  sale: Colors.RED_500, 'عرض': Colors.RED_500,
  new: Colors.EMERALD_600, 'جديد': Colors.EMERALD_600,
  dashpass: Colors.BRAND, popular: Colors.BRAND, 'الأكثر طلبًا': Colors.BRAND,
  'خالٍ من السكر': Colors.PRIMARY_DARK, 'مكافأة': Colors.GOLD,
};
...
export default function ProductCard({ badge, currency = 'د.ل ', image, title, titleEn, productId, onAddToCart, onWishlist, onOpenProduct, originalPrice, price, rating, style }: ProductCardProps) {
    ...
    <View style={[styles.card, style]}>
      ...
      <TouchableOpacity style={styles.content} activeOpacity={0.85} onPress={handleOpen}>
        ...
        <View style={styles.bottomRow}>
          ...
          <TouchableOpacity style={[styles.cartButton, added && styles.cartButtonAdded]} onPress={handleAdd} activeOpacity={0.7} accessibilityLabel={added ? `${title} تمت الإضافة` : `أضف ${title} إلى السلة`}>
...
const styles = StyleSheet.create({
  card: { width: '100%', backgroundColor: Colors.WHITE, borderRadius: Radius.LG, overflow: 'hidden', shadowColor: '#0A1B2A', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.06, shadowRadius: 12, elevation: 3 },
  imageArea: { height: 130, backgroundColor: Colors.PRIMARY_LIGHT, alignItems: 'center', justifyContent: 'center', position: 'relative' },
  imageText: { fontSize: 14, fontWeight: '700', color: Colors.PRIMARY, textAlign: 'center', paddingHorizontal: 10 },
  ratingBadge: { position: 'absolute', top: 10, left: 10, flexDirection: 'row', alignItems: 'center', gap: 3, backgroundColor: 'rgba(10,27,42,0.45)', borderRadius: 13, paddingHorizontal: 8, paddingVertical: 4 },
  ratingText: { color: Colors.WHITE, fontSize: 10, fontWeight: '700' },
  badge: { position: 'absolute', right: 10, top: 10, borderRadius: 6, paddingHorizontal: 7, paddingVertical: 3 },
  badgeText: { color: Colors.WHITE, fontSize: 9, fontWeight: '700' },
  discountPill: { position: 'absolute', bottom: 10, left: 10, backgroundColor: Colors.RED_50, borderRadius: 6, paddingHorizontal: 6, paddingVertical: 3 },
  discountText: { color: Colors.RED_600, fontSize: 10, fontWeight: '700' },
  heartButton: { alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.95)', borderRadius: 16, height: 32, justifyContent: 'center', position: 'absolute', right: 10, top: 48, width: 32, zIndex: 10, shadowColor: '#0A1B2A', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.1, shadowRadius: 3, elevation: 2 },
  pressed: { opacity: 0.65 },
  content: { gap: 4, padding: Spacing.MD, paddingTop: Spacing.SM + 2, paddingBottom: Spacing.MD, alignItems: 'stretch', justifyContent: 'space-between', flex: 1, minHeight: 72 },
  title: { color: Colors.BLACK, fontSize: 14, fontWeight: '700', lineHeight: 18, textAlign: 'right' },
  ...
  priceContainer: { flexDirection: 'row-reverse', alignItems: 'baseline', gap: 6 },
  ...
  cartButton: { width: 34, height: 34, borderRadius: 17, backgroundColor: Colors.PRIMARY, alignItems: 'center', justifyContent: 'center' },
  cartButtonAdded: { backgroundColor: Colors.EMERALD_600 },
});
