  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        <View style={styles.successCircle}><CheckCircle2 size={56} color={Colors.WHITE} strokeWidth={2} /></View>
        ...
        {order && (
          <View style={styles.itemsCard}>
            <Text style={styles.itemsTitle}>تفاصيل الطلب</Text>
            {order.items.map((item) => (
              <View key={item.id + (item.options ?? '')} style={styles.itemRow}>
                <View style={styles.itemInfo}><Text style={styles.itemName}>{item.name}</Text>{item.options ? <Text style={styles.itemOptions}>{item.options}</Text> : null}<Text style={styles.itemQty}>الكمية: {item.quantity}</Text></View>
                <Text style={styles.itemPrice}>{(item.price * item.quantity).toFixed(2)} د.ل</Text>
              </View>
            ))}
            <View style={styles.itemsDivider} />
            <View style={styles.totalRow}><Text style={styles.totalLabel}>الإجمالي</Text><Text style={styles.totalValue}>{params.total} د.ل</Text></View>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  container: { flex: 1 },
  scrollContent: { paddingHorizontal: Spacing.LG, paddingTop: Spacing.XXL, paddingBottom: Spacing.XXL, alignItems: 'stretch' },
  successCircle: { width: 96, height: 96, borderRadius: 48, backgroundColor: Colors.EMERALD_600, alignItems: 'center', justifyContent: 'center', alignSelf: 'center', marginBottom: Spacing.MD, shadowColor: Colors.EMERALD_600, shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.3, shadowRadius: 16, elevation: 8 },
  ...
  actionBtn: { flex: 1, flexDirection: 'row-reverse', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: Colors.WHITE, borderRadius: Radius.PILL, paddingVertical: Spacing.MD, borderWidth: 1.5, borderColor: Colors.PRIMARY },
