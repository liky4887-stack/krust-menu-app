      ...
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 160 }}>
          ...
          {items.map((item) => (
            <View key={item.id + (item.options ?? '')} style={styles.cartRow}>
              <View style={styles.itemImage}>
                {item.imageUrl ? (
                  <Image source={{ uri: item.imageUrl }} style={styles.itemImagePhoto} resizeMode="cover" />
                ) : (
                  <Text style={styles.itemImageText} numberOfLines={2}>{item.name}</Text>
                )}
              </View>
              ...
              <View style={styles.itemActions}>
                <TouchableOpacity
                  style={styles.qtyBtn}
                  onPress={() => decrement(item.id, item.options)}
                  activeOpacity={0.7}
                >
                ...
                <TouchableOpacity
                  style={styles.qtyBtn}
                  onPress={() => increment(item.id, item.options)}
                  activeOpacity={0.7}
                >
              ...
              <TouchableOpacity
                ...
                activeOpacity={0.7}
              >
                <Trash2 size={16} color={Colors.RED_500} strokeWidth={2} />
              </TouchableOpacity>
            </View>
          ))}
        </View>
        ...
        <View style={styles.couponCard}>
          ...
          <View style={styles.couponInput}>
            <Text style={styles.couponPlaceholder}>أدخل كود الخصم</Text>
          </View>
          <TouchableOpacity style={styles.couponBtn} activeOpacity={0.7}>
        ...
        <View style={styles.billCard}>
          ...
          <View style={styles.billRow}>
            ...
            <Text style={styles.billValue}>{subtotal.toFixed(2)} د.ل</Text>
          </View>
          <View style={styles.billRow}>
            <Text style={styles.billLabel}>رسوم التوصيل</Text>
            <Text style={styles.billValue}>{deliveryFee.toFixed(2)} د.ل</Text>
          </View>
          <View style={styles.billRow}>
            <Text style={styles.billLabel}>الضريبة (٨٪)</Text>
            <Text style={styles.billValue}>{tax.toFixed(2)} د.ل</Text>
          </View>
          <View style={styles.billDivider} />
          <View style={styles.billRow}>
            <Text style={styles.billTotalLabel}>الإجمالي</Text>
            <Text style={styles.billTotalValue}>{total.toFixed(2)} د.ل</Text>
          </View>
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  itemImagePhoto: { width: '100%', height: '100%', borderRadius: Radius.MD },
